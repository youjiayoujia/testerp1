<div class="list-group">
    @foreach(Config::get('navigation') as $key => $navigation)
        ...
        @if(isset($navigation['subnavigations']))
            <div class="collapse list-group {{ Request::segment(1) === $navigation['location'] ? 'in' : '' }}" id="subNav{{ $key+1 }}">
                @foreach($navigation['subnavigations'] as $subnavigation)
                    <a class="list-group-item {{ Request::segment(2) === $subnavigation['location'] ? 'list-group-item-info' : '' }}" href="{{ route($subnavigation['url']) }}">
