...
class CreateWarehousePosition extends Migration
{
    ...
    public function up()
    {
        Schema::create('warehouse_positions', function (Blueprint $table) {
            ...
            $table->float('length')->comment('长')->default(50);
            $table->float('width')->comment('宽')->default(50);
            $table->float('height')->comment('高')->default(50);
            $table->enum('is_available', ['0', '1'])->comment('库位是否启用')->default('1');
            $table->timestamps();
            $table->softdeletes();
        });
    }
