    ...
    <div class="form-group col-lg-4">
     ...
     <div class="form-group col-lg-4">
        ...
        @foreach(config('purchase.purchaseItem.active') as $key => $v)
               @if($abnormal->productAbnormal->type == $key) {{$v}} @endif
        @endforeach    
    </div>
    <div class="form-group col-lg-4">
        <label >处理状态:</label>
        <select name="status"  >
        @if($abnormal->productAbnormal->type==1)
        @foreach(config('purchase.productAbnormal.status.1') as $key => $v)
               <option value="{{$key}}" @if($abnormal->productAbnormal->type==$key) selected @endif>{{$v}}</option>
        @endforeach
        @elseif($abnormal->productAbnormal->type==2)
        @foreach(config('purchase.productAbnormal.status.2') as $key => $v)
               <option value="{{$key}}" @if($abnormal->productAbnormal->type==$key) selected @endif>{{$v}}</option>
        @endforeach
        @elseif($abnormal->productAbnormal->type==3)
        @foreach(config('purchase.productAbnormal.status.3') as $key => $v)
               <option value="{{$key}}" @if($abnormal->productAbnormal->type==$key) selected @endif>{{$v}}</option>
        @endforeach
        @elseif($abnormal->productAbnormal->type==4)
     ...
     @if($abnormal->productAbnormal->type ==1)
     <div class="form-group col-lg-4" >
        <label for="URL">主供应商：</label>
        
    </div>
    @endif
    @if($abnormal->productAbnormal->type ==2)
     <div class="form-group col-lg-4" >
        <label for="URL">预计到货日期：</label>
        <input id="waiting_date" class='form-control' name='arrival_time' type="text" placeholder='预计到货时间' value="{{ $abnormal->productAbnormal->arrival_time }}">
    </div>
    @endif
    @if($abnormal->productAbnormal->type ==3)
    <div class="form-group col-lg-4" >
        <label for="URL">异常备注：</label>
        	<textarea name="remark" cols="20" rows="5" style="width:590;hight:140">  
            	{{$abnormal->productAbnormal->remark}}         
        	</textarea>
    </div>
    @endif
    @if($abnormal->productAbnormal->type == 4)
    <div class="form-group col-lg-4">
