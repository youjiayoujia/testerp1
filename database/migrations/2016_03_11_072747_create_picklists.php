...
class CreatePicklists extends Migration
{
    ...
    public function up()
    {
        Schema::create('picklists', function (Blueprint $table) {
            ...
            $table->enum('status', ['NONE', 'PICKING', 'PICKED', 'INBOXING', 'INBOXED', 'PACKAGEING', 'PACKAGED'])->comment('拣货单状态')->default('NONE');
            ...
            $table->integer('inbox_by')->comment('分拣人')->default(0);
            $table->timestamp('inbox_at')->comment('分拣时间');
            $table->integer('pack_by')->comment('包装人')->default(0);
