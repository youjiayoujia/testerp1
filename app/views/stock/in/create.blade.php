    ...
    <div class="form-group">
        ...
        <select name='typeof_stockin' class='form-control'>
            @foreach($data as $stockin_name)
                <option value={{ $stockin_name }} {{ old('typeof_stockin') == $stockin_name ? 'selected' : '' }}>{{ $stockin_name }}</option>   
            @endforeach
        </select>
    </div>
    <div class="form-group">
        <label for="typeof_stockin_id">入库来源id</label>
        <input type='text' class="form-control" id="typeof_stockin_id" placeholder="入库来源id" name='typeof_stockin_id' value="{{ old('typeof_stockin_id') }}">
    </div>
@stop