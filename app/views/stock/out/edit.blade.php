...
@section('formAction') {{ route('stockOut.update', ['id' => $model->id]) }} @stop
@section('formBody')
    ...
    <div class='row'>
        <div class="form-group col-lg-6">
            ...
            <select name='type' class='form-control'>
                @foreach($data as $stockout_key => $stockout_val)
                    <option value="{{ $stockout_key }}" {{ old('type') ? (old('type') == $stockout_key ? 'selected' : '') : ($model->
        ...
        <div class="form-group col-lg-6">
            ...
            <input type='text' class="form-control" id="relation_id" placeholder="出库来源id" name='relation_id' value="{{ old('relation_id') ? old('relation_id') : $model->relation_id }}">
        </div>
    </div>
