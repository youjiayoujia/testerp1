...
class PurchaseOrderController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($response['data'] as $key => $vo) {
            ...
            $response['data'][$key]['purchase_post_num'] = PurchasePostageModel::where('purchase_order_id',
                $vo->id)->count();
            $response['data'][$key]['purchase_post111'] = PurchasePostageModel::where('purchase_order_id',
                $vo->id)->first();
                    ...
                    if (!empty($post)) {
                        foreach ($post as $num => $val) {
                            ...
                            if ($num == 0) {
                                ...
                                PurchasePostageModel::where('post_coding',
                                    $val['post_coding'])->update(['postage' => $val['postage']]);
                            }
            ...
            foreach ($data['arr'] as $k => $v) {
                if ($v['id']) {
                    ...
                    if ($item['purchase_cost'] > 0.6 * $itemPurchasePrice && $item['purchase_cost'] < 1.3 * $itemPurchasePrice) {
                        ...
                        ItemModel::where('sku',
                            $purchaseItem->sku)->update(['purchase_price' => $item['purchase_cost']]);
                    } else {
