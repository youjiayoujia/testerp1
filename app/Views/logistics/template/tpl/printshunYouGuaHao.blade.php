...
<div id="main">
    <div id="main_border">
        ...
        <div style="width:100%; border-top:1px solid; border-bottom:1px solid; line-height:10px;">
            <table style="width:100%; margin:auto;" class="address">
                <tr>
                    ...
                    <th align="right">{{ $model->shipping_firstname . ' ' . $model->shipping_lastname }}</th>
                </tr>

                <tr>
                    <td align="left"></th>
                    <th align="right">{{ $model->shipping_address . ' ' . $model->shipping_address1 }}</th>
                </tr>

                <tr>
                    <td align="left"></th>
                    <th align="right">{{ $model->shipping_city }}{{ $model->shipping_state }}{{ $model->shipping_zipcode }}</th>
                </tr>

                <tr>
                    <td align="left"></th>
                    <th align="right">{{ $model->country ? $model->country->cn_name : '' }}
                                      {{ $model->country ? $model->country->code : '' }}</th>
                </tr>

                <tr>
                    <td align="left">Tel:</th>
                    <th align="right">{{ $model->shipping_phone }}</th>
                </tr>
            </table>
        </div>
        ...
        <table style="width:100%; border-collapse:collapse; border:medium none;" >
            ...
            <tr>
                <th style="border:1px solid #000;text-align:left;">{{ $model->items ? $model->items->sum('quantity') : 0 }} x {{ $model->items ? $model->items->sum('quantity') : 0 }}</th>
                <th style="border:1px solid #000;">{{ $model->total_weight }}</th>
                <th style="border:1px solid #000;">{{ $model->total_weight }}USD</th>
            </tr>
        ...
        <div style="padding:3px;">
            ...
            <div style="text-align:right; width: 97%;">
                <span style="font-size:15px;font-weight:bold;">【{{ $model->logistics_id }}】</span>
                <b>SLME</b> <?php echo date('Y-m-d'); ?>
            </div>
        </div>
        ...
        <div style="padding:3px 6px; border-top:1px solid;">
            <div class="f_l">
                ...
                <p style="text-align:right;">{{$model->id}}</p>
            </div>
