...
class StockModel extends BaseModel
{
    ...
    public function in($quantity, $amount)
    {
        $this->all_quantity += $quantity;
        $this->available_quantity += $quantity;
        $this->amount += $amount;
        $this->save();
        $this->stockIn()->create($arr);
    }

    public function createNew($item_id, $warehoustPosistionId, $quantity, $amount)
    {
        DB::beginTransaction();
        try {
            $stock = $this->where(['item_id'=>$item_id, 'warehouse_position_id'=>$warehousePositionId])->get();
            $len = $stock->count();
            if($len >= 2)
                throw new Exception();
            $stock = $stock->first();
            $stock->all_quantity = $quantity;
            $stock->available_quantity = $quantity;
            $stock->save();
            $arr['stock_id'] = $stock->id;
            $stock->stockIn()->create($arr);
        } catch (Exception $e) {
            DB::rollback();
            throw new Exception('sku对应库位最大数量超过了2个');
        }
