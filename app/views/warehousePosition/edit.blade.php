...
@section('formBody')
    ...
    <div class="form-group">
        ...
        <select name='warehouses_id' class='form-control'>
            @foreach($warehouses as $warehouse)
                <option value="{{ $warehouse->id }}" {{ old('warehouses_id') ? (old('warehouses_id') == $wareshouse->id ? 'selected' : '') : ($warehouse->id  == $warehousePosition->warehouses_id ? 'selected' : '') }}>
                    {{ $warehouse->name }}
                </option>
            @endforeach    
        </select>
    ...
    <div class="form-group">
        ...
        <div class='radio'>
            <label>
                <input type='radio' name='size' value='小' {{ old('size') ? old('size') == '小' ? 'checked' : '' : $warehousePosition->size == '小' ? 'checked' : '' }}>小
            </label>   
        </div>
        <div class='radio'>
            <label>
                <input type='radio' name='size' value='中' {{ old('size') ? old('size') == '中' ? 'checked' : '' : $warehousePosition->size == '中' ? 'checked' : '' }}>中
            </label>
        </div> 
        <div class='radio'>
            <label>
                <input type='radio' name='size' value='大' {{ old('size') ? old('size') == '大' ? 'checked' : '' : $warehousePosition->size == '大' ? 'checked' : '' }}>大
            </label>
        </div>       
    </div>
    <div class="form-group">
        ...
        <div class='radio'>
            <label>
                <input type='radio' name='is_available' value='Y' {{ old('is_available') ? old('is_available') == 'Y' ? 'checked' : '' : $warehousePosition->is_available == 'Y' ? 'checked' : '' }}>启用
            </label>   
        </div>
        <div class='radio'>
            <label>
                <input type='radio' name='is_available' value='N' {{ old('is_available') ? old('is_available') == 'N' ? 'checked' : '' : $warehousePosition->is_available == 'N' ? 'checked' : '' }}>不启用
            </label>
        </div>    
    </div>
@stop