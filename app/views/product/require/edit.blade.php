...
@section('formBody')
    ...
    <div class="form-group">
     ...
     <div class="form-group">
        ...
        <input type='text' class="form-control" id="competition_url" placeholder="竞争产品url" name='competition_url' value="{{ old('competition_url') ? old('competition_url') : $productRequire->competition_url }}">
    </div>
     <div class="form-group">
        <label for="remark" class='control-label'>需求备注</label>
        <input type='text' class="form-control" id="remark" placeholder="需求备注" name='remark' value="{{ old('remark') ? old('remark') : $productRequire->remark }}">
    </div>
    <div class='form-group'>
        <label for="expected_date">期待上传日期</label>
        <input id="expected_date" name='expected_date' type="text" value=" {{ old('expected_date') ? old('expected_date') : $productRequire->expected_date }}">
    </div>
    <div class="form-group">
...
<script type='text/javascript'>
    window.onload = function(){
        var buf = new Array();
        buf[0] = "{{ old('province') ? old('province') : $productRequire->province }}";
        buf[1] = "{{ old('city') ? old('city') : $productRequire->city }}";
        init(buf[0],buf[1]);
